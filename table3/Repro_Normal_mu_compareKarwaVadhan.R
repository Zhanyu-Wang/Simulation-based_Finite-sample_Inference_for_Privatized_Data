...
sdp_fun <- function(data_randomness, privacy_noises, sa, mu) {
  ...
  s1 <- mean(data_clamp) + (upper_clamp - lower_clamp) / (n * ep/2) * privacy_noises[1]
  s2 <- var(data_clamp) + (upper_clamp - lower_clamp) ^ 2 / (n * ep/2) * privacy_noises[2]
  return(c(s1, s2))
}
...
sdp_vec <- function(data_randomness, privacy_noises, sa, mu) {
  ...
  data <- sa * data_randomness + mu
  data_clamp <- pmax(pmin(data, upper_clamp), lower_clamp)
  s1 <- apply(data_clamp, 1, mean) + (upper_clamp - lower_clamp) / (n * ep/2) * privacy_noises[, 1]
  s2 <- apply(data_clamp, 1, var) + (upper_clamp - lower_clamp) ^ 2 / (n * ep/2) * privacy_noises[, 2]
  return(cbind(s1, s2))
}
...
results <- foreach(
  ...
  set.seed(rep)
  data_randomness <- rlaplace(n)
  privacy_noises <- rlaplace(n = 2)
  dp_statistic <- sdp_fun(data_randomness, privacy_noises, population_sigma, population_mu)
  
  data_randomness <- matrix(rlaplace(n * R_synthetic), ncol = n, nrow = R_synthetic)
  privacy_noises <- matrix(rlaplace(R_synthetic * 2), ncol = 2, nrow = R_synthetic)
...
description_str <- paste("compareKarwa_repro_mu-mu=", population_mu, 
...
# "V1"
# "coverage","0.989"
# "se_coverage","0.003298333"
# "mean_width","0.2003709"
# "se_width","0.0005074757"
# "median_width","0.1994246"
# "Total.time","21.96988 mins"
