reps <- 1000

R <- 200
n = 500
alpha=.05
ep=2
shape1=1/2
shape2=1/2
beta0=1/2
beta1=beta1_true=2

set.seed(42)
dir.create(file.path('.', 'results/logistic'), showWarnings = FALSE, recursive = TRUE)
dir.create(file.path('.', 'results/summary'), showWarnings = FALSE, recursive = TRUE)

###

lower = function(x){
  bottom = (x+1)^2-1
  middle = x-1/4
  top = x^2
  return(bottom*(x<=-1/2)+middle*(x>-1/2 & x<1/2)+top*(x>=1/2))
}

upper = function(x){
  bottom = -x^2
  middle = x+1/4
  top = -(x-1)^2+1
  return(bottom*(x<=-1/2)+middle*(x>-1/2 & x<1/2)+top*(x>=1/2))
}


theta = c(beta1,beta0,log(shape1),log(shape2))
# nuisance = c(beta0,shape1,shape2)
nuisance = c(beta0,log(shape1),log(shape2))



###################################################
# automatic install of packages if they are not installed already
list.of.packages <- c(
  "foreach",
  "doSNOW",
  "ddalpha",
  "tictoc"
)
new.packages <- list.of.packages[!(list.of.packages %in% 
                                     installed.packages()[,"Package"])]
if(length(new.packages) > 0){
  install.packages(new.packages, dep=TRUE)
}

# loading packages
for(package.i in list.of.packages){
  suppressPackageStartupMessages(
    library(
      package.i,
      character.only = TRUE
    )
  )
}

# use doSNOW for parallel computing
n.cores <- min(124, parallel::detectCores() - 1)
cl <- makeSOCKcluster(n.cores)
registerDoSNOW(cl)

# # show progress bar
pb <- txtProgressBar(max = reps, style = 3)
progress <- function(n) setTxtProgressBar(pb, n)
opts <- list(progress=progress)
# opts <- list()

###################################################


expit = function(x){
  return(exp(x)/(1+exp(x)))
}


ObjPertLog = function(ep,norm,q,X,Y,N){
  m = dim(X)[2]-1
  lambda = (1/4)*(m+1)
  xi=1
  b=rep(0,m+1)
  if(ep==0){
    Delta=0
  } else {
    if(norm==1){
      xi =(m+1)*2
      #b = rlaplace(n=m+1,s=xi/(q*ep))
    }
    if(norm==2){
      xi = sqrt(m+1)*2
      #norm = rgamma(n=1,shape=m+1,rate = ep*q/(xi))
      #vector = rnorm(n=m+1,m=0,s=1)
      #b=vector*norm/sqrt(sum(vector^2))
    }
    if(norm==3){# 3 represents infinity.
      xi =2
      #norm = rgamma(n=1,shape = m+2,rate = ep*q/(xi))
      #uniform =runif(n=m+1,-1,1)
      #b = (xi/(q*ep))*N#uniform*norm
    }
    b = (xi/(q*ep))*N
    
    Delta = lambda/(exp((1-q)*ep)-1)
  }
  
  
  obj = function(beta){
    return( -(1/n)*sum(Y*(X%*%beta) - log(1+exp(X%*%beta)))+Delta/(2*n)*sum(beta^2) + sum(b*beta)/n)
  }
  grad = function(beta){
    logLike = (apply((X)*matrix(Y-exp(X%*%beta)/(1+exp(X%*%beta)),nrow=n,ncol=m+1),2,function(x) sum(x)))
    # 2 means cols
    return( -(1/n)*logLike    + (Delta/n)*beta + b/n)
  }
  #min = optim(par = rep(0,m+1),fn=obj,gr = grad,method = "L-BFGS-B")
  min = optim(par = rep(0,m+1),fn=obj,gr = grad,method = "BFGS")
  
  if(min$convergence!=0){
    print("Obj-Pert did not converge") 
  }
  return(min$par)
}###   END OBJ  PERT

sdp_fun = function(seed,U,N1,N2,ep,theta,n){
  beta1=theta[1]
  beta0=theta[2]
  shape1=exp(theta[3])
  shape2=exp(theta[4])
  
  ep1 = ep*.9
  ep2 = (ep-ep1)
  n=length(seed)
  x = qbeta(seed,shape1,shape2)
  x2 = 2*x-1
  Y = U<=expit(beta0+beta1*x2)
  X = matrix(c(rep(1,n),x2),nrow=n,ncol=2)
  
  s12 = ObjPertLog(ep=ep1,norm=3,q=.85,X=X,Y=Y,N=N1)
  s3 = mean(x)+(1/(ep2*n))*N2[1]
  s4 = mean(x^2)+(1/(ep2*n))*N2[2]
  
  return(c(s12,s3,s4))
}
# s_dp = sdp_fun(seed,N,l,u,ep)



sdp_vec = function(seed,U,N1,N2,ep,theta,n){
  s = matrix(rep(0,R*4),nrow=R,ncol=4)
  ## below is tricky to vectorize
  for(i in 1:R){
    s[i,] = sdp_fun(seed[i,],U[i,],N1[i,],N2[i,],ep,theta,n)
  }
  return(s)
}
# sdp_vec2 = function(seed,U,N1,N2,ep,theta,n){
#   s <- foreach(
#     i = 1:R,
#     .combine = 'rbind',
#     .packages=c('ddalpha'),
#     .export=c('expit','ObjPertLog','sdp_fun'),
#     .options.snow=opts
#   ) %dopar% {
#     sdp_fun(seed[i,],U[i,],N1[i,],N2[i,],ep,theta,n)
#   }
#   return(s)
# }
# tic(); a=sdp_vec(seed,U,N1,N2,ep,theta,n); toc()
# tic(); a=sdp_vec(seed,U,N1,N2,ep,theta,n); toc()
# tic(); a=sdp_vec(seed,U,N1,N2,ep,theta,n); toc()
# tic(); a=sdp_vec2(seed,U,N1,N2,ep,theta,n); toc()
# tic(); a=sdp_vec2(seed,U,N1,N2,ep,theta,n); toc()
# tic(); a=sdp_vec2(seed,U,N1,N2,ep,theta,n); toc()


# 
# score = function(nuisance,beta1,seed,U,N1,N2,s_dp,ep,n){
#   theta = c(beta1,nuisance)
#   synth = sdp_vec(seed,U,N1,N2,ep,theta)
#   
#   synth = rbind(synth,s_dp)
#   #depth.contours(synth, depth = "halfspace", main = "halfspace")
#   #points(s_dp[1],s_dp[2])
#   
#   #D_synth = depth.sample(synth,synth)
#   #D_synth = D_synth$Simpl
#   
#   D_synth = depth.Mahalanobis(synth,synth)
#   ### could append s_dp at the end of 2nd synth
#   
#   r = rank(D_synth)[R+1]
#   return(-r)
#   
# }
getRank = function(theta,seed,U,N1,N2,s_dp,ep,n){
  synth = sdp_vec(seed,U,N1,N2,ep,theta,n)
  synth = rbind(synth,s_dp)
  
  D_synth = depth.Mahalanobis(synth,synth)
  D_sort = sort(D_synth)
  r = sum(D_sort<=D_synth[R+1])
  return(r)
}



scoreMCE_theta = function(theta,seed,U,N1,N2,s_dp,ep,n){
  gc()
  t = (floor((alpha)*(R+1))+1)
  synth = sdp_vec(seed,U,N1,N2,ep,theta,n)
  synth = rbind(synth,s_dp)
  
  D_synth = depth.Mahalanobis(synth,synth)
  D_sort = sort(D_synth)
  r = sum(D_sort<=D_synth[R+1])
  s = r + D_synth[R+1]
  return(-s)
}

scoreMCE_nuisance = function(nuisance,beta1,seed,U,N1,N2,s_dp,ep,n){
  theta = c(beta1,nuisance)
  print(theta)
  return(scoreMCE_theta(theta,seed,U,N1,N2,s_dp,ep,n))
}


accept = function(beta1,seed,U,N1,N2,s_dp,ep,n){
  rank0 = getRank(c(beta1,nuisance),seed,U,N1,N2,s_dp,ep,n)
  if(rank0>=(floor((alpha)*(R+1))+1))
    return(TRUE)
  
  #n_hat = nuisance_hat2(beta1=beta1,s_dp,Delta,init=nuisance)
  #rank1 = getRank(c(beta1,n_hat),ux,uy,N,Delta,s_dp,ep,n)
  #if(rank1>=(floor((alpha)*(R+1))+1))
  #  return(TRUE)
  
  ### optim fails with larger n...
  opt = optim(par =nuisance,fn = scoreMCE_nuisance,
              method="L-BFGS-B",lower=c(-10,-5,-5),upper=c(10,5,5),
              beta1=beta1,seed=seed,U=U,N1=N1,N2=N2,s_dp=s_dp,ep=ep,n=n
  )
  #opt = optim(par = 2,fn = score,lower=0,method="L-BFGS-B",mu=m)
  #rank = getRank(c(beta1,opt$par),ux,uy,N,Delta,s_dp,ep,n)
  #print(opt$value)
  #print(opt$par)
  
  return((-opt$value)>=(floor((alpha)*(R+1))+1))
}


getCI = function(seed,U,N1,N2,s_dp,ep,n){
  
  if(accept(beta1=beta1_true,seed,U,N1,N2,s_dp,ep,n))
    theta_m=beta1_true
  else{
    opt = optim(par=theta,fn=scoreMCE_theta,
                method="L-BFGS-B",lower=c(-10,-10,-5,-5),upper=c(10,10,5,5),
                seed=seed,U=U,N1=N1,N2=N2,s_dp=s_dp,ep=ep,n=n)
    if(opt$value<=0)#getRank(opt$par,ux,uy,N,Delta,s_dp,ep,n)>=(floor((alpha)*(R+1))+1))
      theta_m=opt$par[1]
    else{
      print("Cannot find a point in CI")
      return(c(0,0))
    }
    #opt2 = optim(par=1.5,fn=score,lower=-5,upper=10,method="Brent",seed=seed,N=N,n=n,s_dp=s_dp)
    #theta_m=opt2$par
  }
  ### binary search:
  theta_0=-10
  theta_1=10
  tol=10^-3
  
  ##get lower bound:
  lower =theta_0
  middle=theta_m
  while(middle-lower>tol){
    tic()
    print(c(lower,middle))
    proposal = (middle+lower)/2
    
    if(accept(proposal,seed,U,N1,N2,s_dp,ep,n))
      middle=proposal
    else{
      lower=proposal
    }
    toc()
  }
  #lower
  #accept(lower)
  #accept(middle)
  
  
  ##get upper bound:
  middle =theta_m
  upper=theta_1
  while(upper-middle>tol){
    print(c(middle,upper))
    proposal = (upper+middle)/2
    #synth = M(e,N,theta=proposal,U,ep)
    #interval=emp.hpd(synth)
    #interval = quantile(synth,c(.025,.975))
    if(accept(proposal,seed,U,N1,N2,s_dp,ep,n))
      middle=proposal
    else{
      upper=proposal
    }
  }
  
  return(c(lower,upper))
}


n_list = c(100, 200, 500, 1000, 2000)
ep_list = c(0.1, 0.3, 1, 3, 10)

for(ep in ep_list){
  for(n in n_list){
  filename2 = paste("./results/summary/Repro_Logistic",
                    "-shape1_", shape1, "-shape2_", shape2,
                    "-beta0_", beta0, "-beta_1", beta1, "-R_", R, "-n_", n,
                    "-alpha_", alpha, "-ep_", ep, "-reps_", reps, ".csv", sep="")
  print(filename2)
  if (file.exists(filename2)) {
    next
  }
  CIs <- foreach(
    rep = 1:1000,
    .combine = 'rbind',
    .packages=c('ddalpha', 'tictoc'),
    .options.snow=opts
  ) %dopar% {
    
    filename = paste("./results/logistic/Repro_Logistic", 
                    "-shape1_", shape1, "-shape2_", shape2, 
                    "-beta0_", beta0, "-beta_1", beta1, "-R_", R, "-n_", n,
                    "-alpha_", alpha, "-ep_", ep, "-reps_", reps, "-", rep, ".csv", sep="")
    if (!file.exists(filename)) {
      set.seed(rep)
      tic()
      rejection_num = 100
      seed = runif(n,min=0,max=1)
      U = runif(n,min=0,max=1)
      
      U1 = runif(2,min=-1,max=1)
      G1 = rgamma(1,shape=3,rate = 1)
      N1 = G1*U1
      
      W1 = matrix(runif(rejection_num*2*1,min=-1,max=1),nrow=rejection_num*1,ncol=2)
      index = which(W1[,2]>=lower(W1[,1]) & W1[,2]<=upper(W1[,1]))
      
      U2 = as.numeric(W1[index[1],])
      G2 = rgamma(1,shape=3,rate = 1)
      N2 = G2*U2
      
      s_dp = sdp_fun(seed,U,N1,N2,ep,theta,n)
      
      
      seed = matrix(runif(n*R,min=0,max=1),nrow=R)
      U = matrix(runif(n*R,min=0,max=1),nrow=R)
      
      U1 = matrix(runif(2*R,min=-1,max=1),nrow=R)
      G1 = rgamma(R,shape=3,rate = 1)
      N1 = G1*U1
      
      W1 = matrix(runif(rejection_num*R*2,min=-1,max=1),nrow=rejection_num*R,ncol=2)
      index = which(W1[,2]>=lower(W1[,1]) & W1[,2]<=upper(W1[,1]))
      
      U2 = W1[index[1:R],]
      G2 = rgamma(R,shape=3,rate = 1)
      N2 = G2*U2
      
      CI = getCI(seed,U,N1,N2,s_dp,ep,n)
      time_elapsed = toc()
      result = data.frame(c(CI, time_elapsed$toc - time_elapsed$tic))
      colnames(result) = "1"
      rownames(result) = c("l", "h", "t")
      write.csv(result, filename) # for saving the intermediate results
      t(result)
    }
  }
  
  CIs = c()
  for (rep in c(1:reps)){
    filename = paste("./results/logistic/Repro_Logistic",
                    "-shape1_", shape1, "-shape2_", shape2,
                    "-beta0_", beta0, "-beta_1", beta1, "-R_", R, "-n_", n,
                    "-alpha_", alpha, "-ep_", ep, "-reps_", reps, "-", rep, ".csv", sep="")
    csv_file = read.csv(filename, header=TRUE)
    CI = csv_file$X1
    CIs = rbind(CIs, CI)
  }
  write.csv(CIs, filename2)
  }
}
stopCluster(cl)
